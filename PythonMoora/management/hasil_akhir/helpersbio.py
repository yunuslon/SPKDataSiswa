...
def Matrix_TernormalisasiBio(na):
    swa= ListSiswa(na)
    nak=round(Hasil_NilaiAkademikBio(na),5)
    hk=round(Hasil_KarakterBio(na),5)
    hp=round(Hasil_PlombaBio(na),5)
    ht=round(HasilTesBio(na),5)
    hkl=round(Hasil_KelasBio(na),5)
    new = pd.concat([swa,nak,hk,hp,ht,hkl],axis=1)
    return new
###############################Matrix Pembobotan###########################################################

def Matrix_PembobotanBio(na):
    swa= ListSiswa(na)
    nak=round(Hasil_NilaiAkademikBio(na)*(bb[0].nilai_akademik/100),5)
    hk=round(Hasil_KarakterBio(na)*(bb[0].karakter/100),5)
    hp=round(Hasil_PlombaBio(na)*(bb[0].plomba/100),5)
    ht=round(HasilTesBio(na)*(bb[0].hasil_tes/100),5)
    hkl=round(Hasil_KelasBio(na)*(bb[0].kelas/100),5)
...
def Hasil_AkhirBio(na):
    swa= ListSiswa(na)
    nak=Hasil_NilaiAkademikBio(na)*(bb[0].nilai_akademik/100)
    hk=Hasil_KarakterBio(na)*(bb[0].karakter/100)
    hp=Hasil_PlombaBio(na)*(bb[0].plomba/100)
    ht=HasilTesBio(na)*(bb[0].hasil_tes/100)
    hkl=Hasil_KelasBio(na)*(bb[0].kelas/100)
    hp.columns=['pl']
    ht.columns=['htm']
