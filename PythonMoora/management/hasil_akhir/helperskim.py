...
def Matrix_TernormalisasiKim(na):
    swa= ListSiswa(na)
    nak=round(Hasil_NilaiAkademikKim(na),5)
    hk=round(Hasil_KarakterKim(na),5)
    hp=round(Hasil_PlombaKim(na),5)
    ht=round(HasilTesKim(na),5)
    hkl=round(Hasil_KelasKim(na),5)
    new = pd.concat([swa,nak,hk,hp,ht,hkl],axis=1)
    return new
###############################Matrix Pembobotan###########################################################

def Matrix_PembobotanKim(na):
    swa= ListSiswa(na)
    nak=round(Hasil_NilaiAkademikKim(na)*(bb[0].nilai_akademik/100),5)
    hk=round(Hasil_KarakterKim(na)*(bb[0].karakter/100),5)
    hp=round(Hasil_PlombaKim(na)*(bb[0].plomba/100),5)
    ht=round(HasilTesKim(na)*(bb[0].hasil_tes/100),5)
    hkl=round(Hasil_KelasKim(na)*(bb[0].kelas/100),5)
...
def Hasil_AkhirKim(na):
    swa= ListSiswa(na)
    nak=Hasil_NilaiAkademikKim(na)*(bb[0].nilai_akademik/100)
    hk=Hasil_KarakterKim(na)*(bb[0].karakter/100)
    hp=Hasil_PlombaKim(na)*(bb[0].plomba/100)
    ht=HasilTesKim(na)*(bb[0].hasil_tes/100)
    hkl=Hasil_KelasKim(na)*(bb[0].kelas/100)
    hp.columns=['pl']
    ht.columns=['htm']
